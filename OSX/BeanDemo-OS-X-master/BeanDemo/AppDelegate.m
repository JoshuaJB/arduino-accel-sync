...
- (void)applicationDidFinishLaunching:(NSNotification *)aNotification
{
    ...
    //formatting the numbers that indicate acc data. Variables defined in appdelegate.m
        NSNumberFormatter *formatter = [[NSNumberFormatter alloc] init];
        [formatter setNumberStyle:NSNumberFormatterDecimalStyle];
        [formatter setMaximumFractionDigits:3];
        
        [xAcc setFormatter:formatter]; // sets the color of the text that says accelerometer number
        [xAcc setTextColor:[NSColor brownColor]];
...
- (IBAction)newConnectionMenu:(id)sender
{
        if (bean.state != BeanState_ConnectedAndValidated) {
                NSLog(@"Finding New Beans"); //thats how you print on the terminal window
                ...
                if(self.beanManager.state == BeanManagerState_PoweredOn){ // bean manager is on
...
- (void)sheetDidEnd:(NSWindow *)sheet returnCode:(NSInteger)returnCode contextInfo:(void *)contextInfo
{
        ...
        if( returnCode == NSAlertDefaultReturn )
        {
                ...
                if (selectedRow != -1)
                {
                        ...
                        NSTimer *timer = [NSTimer  scheduledTimerWithTimeInterval:0.2 //this is where we determine the time interval for each read from bean
...
- (void)tableView:(NSTableView *)tableView willDisplayCell:(id)cell forTableColumn:(NSTableColumn *)tableColumn row:(NSInteger)row {
        if ([tableView isEqualTo:scratchTable]) { //edits color of scratch table
...
-(void)bean:(PTDBean*)bean didUpdateTemperature:(NSNumber*)degrees_celsius {
        [self.tempReading setStringValue:[NSString stringWithFormat:@"%@ºC",degrees_celsius]]; //where we are getting the degrees celsius
        //NSLog(@"temp: %@ºC",degrees_celsius);
}

-(void)bean:(PTDBean *)bean didUpdateAccelerationAxes:(PTDAcceleration)acceleration { // where we are getting  acceleration data
